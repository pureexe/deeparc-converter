...
def resize_with_border(image,target_height,target_width):
    ...
    h_new = int(h*scale)
    w_new = int(w*scale)
    scaled_image = cv2.resize(image,(w_new,h_new))
    vertical_border = target_height - h_new
    horizontal_border = target_width - w_new
    top_border = int(vertical_border / 2)
    left_border = int(horizontal_border / 2)
    new_image = np.zeros((target_height,target_width,c))
    new_image[top_border:top_border+h_new,left_border:left_border+w_new] = scaled_image
    return new_image
